...
export class TimeSeriesPath {
     ...
     */
    forwardFindIndex(targetTimestamp, mode = IndexMode.Exclusive) {
        // { from: number; to: number; between: boolean }
        let CompareReturn;
        (function (CompareReturn) {
            /** The timestamp is before the target */
            CompareReturn[CompareReturn["TimestampBeforeTarget"] = 0] = "TimestampBeforeTarget";
            /** The timestamp is at the target */
            CompareReturn[CompareReturn["TimestampAtTarget"] = 1] = "TimestampAtTarget";
            /** The timestamp is after the target */
            CompareReturn[CompareReturn["TimestampAfterTarget"] = 2] = "TimestampAfterTarget";
         ...
         */
        function compare(timestamp, targetTimestamp) {
            ...
            if (timestamp < targetTimestamp) {
                ...
                throw Error(`Logical error in compare function: fromTimestamp = ${timestamp}, targetTimestamp = ${targetTimestamp}`);
            }
        }
                ...
                case CompareReturn.TimestampAtTarget:
                    switch (mode) {
                        ...
                        case IndexMode.DiscontinuityInclusive:
                            ...
                            else if (compare(this.timestamps[timestampCursor + 1], targetTimestamp) ===
                                CompareReturn.TimestampAtTarget)
        ...
        if (count === maxCount) {
            throw Error(`forwardFindIndex was unable to find timestamp ${targetTimestamp} in ${maxCount} attempts`);
        }
    ...
    append(appendedTimeSeriesPath) {
        const returnTimeSeriesPeriod = this.clone();
        const foundIndex = this.forwardFindIndex(appendedTimeSeriesPath.timestamps[0], IndexMode.Exclusive);
     ...
     */
    static multiAppend(appendedTimeSeriesPaths) {
        if (appendedTimeSeriesPaths.length === 0) {
            return null;
        }
        else {
            let returnTimeSeriesPath = new TimeSeriesPath(appendedTimeSeriesPaths[0].dataType, appendedTimeSeriesPaths[0].interpolationMethod);
     ...
     * @returns A new time series path
     */
    replace(timeSeriesPath) {
        var _a, _b, _c;
        const returnTimeSeriesPeriod = this.clone();
        const foundStartIndex = this.forwardFindIndex(timeSeriesPath.timestamps[0], IndexMode.Exclusive);
        const foundEndIndex = this.forwardFindIndex(timeSeriesPath.timestamps[timeSeriesPath.timestamps.length - 1], IndexMode.DiscontinuityInclusive);
        returnTimeSeriesPeriod.timestamps = this.timestamps
