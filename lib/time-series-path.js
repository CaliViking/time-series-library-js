...
class TimeSeriesPath {
    ...
    validate() {
        ...
        const arraySizeOK = this.timestamps.length === this.values.length && this.timestamps.length === this.statuses.length;
    ...
    operatorTS(operator, arg) {
        ...
        const targetTimestamps = [...new Set((this.timestamps.concat(arg.timestamps)).sort((a, b) => a - b))];
        const targetValues = [];
        const targetStatuses = [];
