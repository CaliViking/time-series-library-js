...
export class TimeSeriesVector {
     ...
     * @param sliceSize The maximum number of time series entries in each object
     */
    slice(fromTimestamp, toTimestamp = this.timestamps[this.timestamps.length], mode = SliceMode.IncludeOverflow) {
        /** An array that will contain all the time series path objects to be returned */
        const returnTimeSeriesVector = new TimeSeriesVector();
        const foundStartIndex = forwardFindIndex(this.timestamps, fromTimestamp, mode === SliceMode.ExcludeOverflow ? IndexMode.ExcludeOverflow : IndexMode.Inclusive);
        const foundEndIndex = reverseFindIndex(this.timestamps, toTimestamp, mode === SliceMode.ExcludeOverflow ? IndexMode.ExcludeOverflow : IndexMode.Inclusive);
